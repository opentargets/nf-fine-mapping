...
workflow LOCUS_ANNOTATION {
    ...
    main:
    ld_annotation_method = params.ld_annotation_method.toString().toLowerCase()
    if (!(ld_annotation_method in ['gentropy', 'hailing_ducks'])) {
        error "Unsupported LD annotation method: ${params.ld_annotation_method}"
    }
    ...
    ch_selected_ld_annotation_input = ch_full_overlap_loci.map { r ->
        ...
        if (ld_annotation_method == 'gentropy') {
            return tuple(
                r.runId,
                r.metas,
    ...
    if (ld_annotation_method == 'gentropy') {
        ch_ld_annotation = GENTROPY_FINE_MAPPING_LOCUS_SET_LD_ANNOTATION(ch_selected_ld_annotation_input)
    } else {
